...
def fix_permissions(somefile):
	"""
	make file accessible when run by apache
	"""
	open(somefile, 'w').close()
	os.chmod(somefile, 0777)
...
def prep_working_directory():
	...
	to_remove = [os.path.join(temp_dir, f) for f in os.listdir(temp_dir)]
	for f in to_remove:
		os.unlink(f)
...
def prep_scp(wavfile):
	scp_fname = os.path.join(temp_dir, 'codetr.scp')
	fix_permissions(scp_fname)
	fw = open(scp_fname, 'w')
	plp_file = os.path.join(temp_dir, "tmp.plp")
	fw.write('{wav_f} {plp_path}\n'.format(wav_f=wavfile, plp_path=plp_file))
	fw.close()
	fw = open(os.path.join(temp_dir,'test.scp'), 'w')
	fw.write('{plp_path}\n'.format(plp_path=plp_file))
	fw.close()
	
def create_plp(hcopy_config) :
	codetr_dir = os.path.join(temp_dir, 'codetr.scp')
	fix_permissions(codetr_dir)
	os.system('/usr/local/bin/HCopy -T 1 -C {hcopy_conf} -S {codetr_loc}'.format(hcopy_conf=hcopy_config, codetr_loc=codetr_dir))
	
def viterbi(input_mlf, word_dictionary, output_mlf, phoneset, hmmdir) :
	codetr_dir = os.path.join(temp_dir, 'codetr.scp')
	test_scp_dir = os.path.join(temp_dir, 'test.scp')
	#fix permissions on test_scp
	fix_permissions(test_scp_dir)

	results_dir = os.path.join(temp_dir, 'aligned.results')
	#fix permissions on aligned.results
	fix_permissions(results_dir)
	...
	#'/usr/local/bin/HVite -T 1 -a -m -I ' + input_mlf + ' -H ' + hmmdir + '/macros -H ' + hmmdir + '/hmmdefs  -S {0} -i '.format(test_scp_dir) + output_mlf + ' -p 0.0 -s 5.0 ' + word_dictionary + ' ' + phoneset + ' > {0}'.format(results_dir)
...
if __name__ == '__main__':
	...
	sr_models = None
	if mypath == None :
		mypath = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "model")
		hmmsubdir = "FROM-SR"
	...
	if os.path.exists("dict.local"):
		cat_dict = "cat {dict_loc} dict.local > {dictionary}".format(dict_loc=os.path.join(mypath, "dict"), dictionary=word_dictionary)
		#os.system(cat_dict)
		#os.system("cat " + mypath + "/dict dict.local > " + word_dictionary)
	else:
		cat_dict = "cat {dict_loc} > {dictionary}".format(dict_loc=os.path.join(mypath, "dict"), dictionary=word_dictionary)
		#os.system("cat " + mypath + "/dict > " + word_dictionary)
	os.system(cat_dict)

	temp_dict = os.path.join(temp_dir, 'dict')
	#fix permissions
	fix_permissions(temp_dict)

	#prepare wavefile: do a resampling if necessary
	tmpwav = os.path.join(os.getcwd(), "tmp", "sound.wav")
	#fix permissions
	fix_permissions(tmpwav)

	SR = prep_wav(wavfile, tmpwav, sr_override, wave_start, wave_end)
	
	if hmmsubdir == "FROM-SR" :
		hmmsubdir = os.path.join(mypath, str(SR))
	...
	create_plp(os.path.join(hmmsubdir, 'config'))
	
	# run Verterbi decoding
	#print "Running HVite..."
	mpfile = os.path.join(mypath,'monophones')
	if not os.path.exists(mpfile) :
		mpfile = os.path.join(mypath,'hmmnames')
		fix_permissions(mpfile)
	viterbi(input_mlf, word_dictionary, output_mlf, mpfile, hmmsubdir)
